...
for (i in c("humpback", "minke")) {
  ...
  p <- ggplot(data = master_dat, aes(x = x, y = y, fill = layer)) +
    ...
    coord_quickmap() +
    facet_wrap(~month) +
    scale_fill_viridis_c(option = "viridis", name = "Overlap", trans = "sqrt") +
  ...
  bidat <- bi_class(master_dat, x = p, y = catch, style = "equal", dim = 3)
  
  pb <- ggplot(data = bidat, aes(x = x, y = y, fill = bi_class)) +
    geom_raster() +
    coord_quickmap() +
    facet_wrap(~month) +
  ...
  png(paste0("./figures/overlap_bi_", which_whale, ".png"),
      width = 150/0.7,
      height = 150/0.7,
      units = "mm",
      res = 700)
